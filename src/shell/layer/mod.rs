...
use smithay_client_toolkit::reexports::{
    ...
    protocols::xdg::shell::client::xdg_popup::XdgPopup,
...
impl LayerShell {
    ...
    where
        State: Dispatch<zwlr_layer_shell_v1::ZwlrLayerShellV1, (), State>
            + LayerShellHandler
            + 'static,
    {
...
pub trait LayerShellHandler: Sized {
    ...
    /// should be dropped.
    fn closed(&mut self, conn: &Connection, qh: &QueueHandle<Self>, layer: &WlrLayerSurface);
...
impl LayerSurfaceData {
    pub fn layer_surface(&self) -> Option<WlrLayerSurface> {
        self.inner.upgrade().map(WlrLayerSurface)
    }
}
