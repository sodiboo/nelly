...
impl XdgShell {
    ...
    where
        State:
            Dispatch<XdgWmBase, (), State> + Dispatch<ZxdgDecorationManagerV1, (), State> + 'static,
    {
        let xdg_wm_base = globals.bind(qh, 1..=Self::API_VERSION_MAX, ())?;
        let xdg_decoration_manager = GlobalProxy::from(globals.bind(qh, 1..=1, ()));
        Ok(Self {
            xdg_wm_base,
            ...
            + 'static,
    {
        ...
        let inner = Arc::new_cyclic(|weak| {
            ...
            let xdg_surface = XdgShellSurface {
                surface,
                xdg_surface,
            };
...
impl ObjectData for PositionerData {
    fn event(
        self: Arc<Self>,
        _: &Backend,
...
impl XdgShellSurface {
        ...
        U: Send + Sync + 'static,
    {
        ...
        let xdg_surface = wm_base
            .bound_global()?
            .get_xdg_surface(surface.wl_surface(), qh, udata);
...
pub trait XdgSurface: WaylandSurface + Sized {
    ...
    fn set_window_geometry(&self, x: u32, y: u32, width: u32, height: u32) {
        self.xdg_surface()
            .set_window_geometry(x as i32, y as i32, width as i32, height as i32);
    }
}
...
macro_rules! delegate_xdg_shell {
    ($(@<$( $lt:tt $( : $clt:tt $(+ $dlt:tt )* )? ),+>)? $ty: ty) => {
        ...
        ::smithay_client_toolkit::reexports::client::delegate_dispatch!($(@< $( $lt $( : $clt $(+ $dlt )* )? ),+ >)? $ty: [
            ::smithay_client_toolkit::reexports::protocols::xdg::decoration::zv1::client::zxdg_toplevel_decoration_v1::ZxdgToplevelDecorationV1: $crate::shell::xdg::window::WindowData
        ] => $crate::shell::xdg::XdgShell);
    };
}
