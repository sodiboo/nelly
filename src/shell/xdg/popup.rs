...
impl Popup {
    ...
    where
        D: Dispatch<wl_surface::WlSurface, SurfaceData>
            ...
            + Dispatch<wp_fractional_scale_v1::WpFractionalScaleV1, SurfaceData>
            + Dispatch<xdg_surface::XdgSurface, PopupData>
            + Dispatch<xdg_popup::XdgPopup, PopupData>
            + PopupHandler
            + 'static,
    {
        let surface = Surface::new(compositor, qh, view_id);
        ...
        match event {
            xdg_surface::Event::Configure { serial } => {
                ...
                let kind = match inner
                    .configure_state
                    .swap(PopupConfigure::STATE_CONFIGURED, Relaxed)
                {
                    PopupConfigure::STATE_NEW => ConfigureKind::Initial,
                    PopupConfigure::STATE_CONFIGURED => ConfigureKind::Reactive,
                    PopupConfigure::STATE_REPOSITION_ACK => ConfigureKind::Reposition {
                        token: inner.pending_token.load(Relaxed),
                    },
                    _ => unreachable!(),
                };

                let config = PopupConfigure {
                    position: (x, y),
                    width,
                    height,
                    serial,
    ...
    fn event(
        ...
        match event {
            ...
            xdg_popup::Event::Repositioned { token } => {
                ...
                inner
                    .configure_state
                    .store(PopupConfigure::STATE_REPOSITION_ACK, Relaxed);
            }
            _ => unreachable!(),
        }
    }
